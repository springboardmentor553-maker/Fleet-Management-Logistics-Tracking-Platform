...
import { Truck, IdCard, Package, Route, MapPin, Wrench, Calendar, Send, CheckCircle2, AlertTriangle, X, RefreshCw } from 'lucide-react'
...
const DashboardHome = ({ vehicles = [], drivers = [], shipments = [], trips = [], maintenanceRecords = [], loading, search, onRefresh }) => {
  ...
  return (
    <>
      <div className="ff-dashboard-intro">
        <div>
          <div className="ff-eyebrow">Operations overview</div>
          <h1 className="ff-dashboard-title">Good day, here is your fleet at a glance.</h1>
          <p className="ff-page-subtitle">Monitor live movement, delivery health, and upcoming service work.</p>
        </div>
        <button className="ff-btn-secondary" type="button" onClick={onRefresh} disabled={loading}>
          <RefreshCw size={14} className={loading ? 'ff-spin' : ''} />
          Refresh data
        </button>
      </div>
        ...
        <div className="ff-widget-card">
          ...
          {totalVehicles > 0 ? (
            ...
            <EmptyState
              ...
              description="Add your first vehicle to start monitoring fleet performance."
              icon={Truck}
            />
          )}
        </div>
      ...
      <div className="ff-bottom-row">
        <div className="ff-widget-card">
          ...
          {recentActivities.length === 0 && (
            <EmptyState title="No recent activity" description="New shipment, trip, vehicle, and driver events will appear here." icon={Calendar} />
          )}
        ...
        <div className="ff-widget-card">
          ...
          {upcomingMaintenance.length === 0 && (
            <EmptyState title="No service due soon" description="Your upcoming maintenance schedule is clear." icon={Wrench} />
          )}
          {upcomingMaintenance.length > 0 && (
