...
def fetch_shipments(db: Session = Depends(get_db)):
    return get_all_shipments(db)


@router.get("/{shipment_id}", response_model=ShipmentResponse)
def fetch_shipment(shipment_id: int, db: Session = Depends(get_db)):
    return get_shipment(shipment_id, db)


@router.put("/{shipment_id}", response_model=ShipmentResponse)
def edit_shipment(shipment_id: int, shipment: ShipmentUpdate, db: Session = Depends(get_db)):
    return update_shipment(shipment_id, shipment, db)


@router.delete("/{shipment_id}")
def remove_shipment(shipment_id: int, db: Session = Depends(get_db)):
