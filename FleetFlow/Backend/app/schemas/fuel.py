...
class FuelRecordBase(BaseModel):
    ...
    fuel_quantity: float = Field(..., gt=0, description="Fuel quantity in Liters")
    fuel_cost: float = Field(..., gt=0, description="Cost of fuel purchased")
    odometer_reading: float = Field(..., ge=0, description="Odometer reading at fuel purchase")
    fuel_station: str = Field(..., description="Name/Location of the fuel station")
    remarks: Optional[str] = Field(None, description="Optional remarks")


class FuelRecordCreate(FuelRecordBase):
    fuel_date: Optional[datetime] = Field(default_factory=datetime.utcnow, description="Date of fueling")
